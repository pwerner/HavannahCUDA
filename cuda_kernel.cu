...
__global__ void playouts_d(const small_board * org_b, game * games, curandState * rng) {
	...
	copy_board_d(org_b,b);
	// perform our move
	perf_move_d(b,g->move,&code);
	// perform playout
	if(playout_d(b,&code,&rng[id])) {
...
__global__ void move_d(small_board *b) {
	...
	code_board_10 code;
	init_d(&code);
	perf_move_d(b,0,&code);
}

// test kernel for 1 playout
__global__ void do_playout_d(small_board *org_b,long seed) {
	small_board sb;
	small_board *b = &sb;
	// get copy of orignal board
	copy_board_d(org_b,b);
	...
	code_board_10 code;
	init_d(&code);
	playout_d(b,&code,&rng);
	copy_board_d(b,org_b);
}

__global__ void setup_rnd_kernel(curandState* const rng,
				 const int seed)
{
...
__device__ void getNeighbours_d(const code_board * code, idx index, neighbours * n) {
	...
	short lineNr = getLine_d(code, index);
	line lineBounds;
...
__device__ byte onMove_d(const small_board * b) {
	return next_d(b) == WHITE ? BLACK : WHITE;
}
...
__device__ void unionSets_d(small_board * b, idx index1, idx index2) {
	idx root1 = findRoot_d(b, index1);
	idx root2 = findRoot_d(b, index2);
	if(root1 == root2) {
		return;
	}
...
__device__ bool playout_d(small_board * b, const code_board * code,curandState * rng) {
	...
	short lastIndex = init_toplay_d(toPlay,b);
	bool win = false;
	while(!win) {
		// no more free moves
		if(lastIndex < 0) {
		...
		win = playoutStep_d(b,code,toPlay, lastIndex--,rng);
		if(win) {
			byte winner = getWinner_d(b);
			if(winner == color) {
				return true;
			}
			return false;
...
__device__ bool playoutStep_d(small_board * b, const code_board * code, idx* toPlay, short lastIndex,curandState * rng) {
	idx index = getIndex_d(toPlay, lastIndex,rng);
	if(ringWin_d(b,index,code)) {
		return true;
	}
	perf_move_d(b,index,code);
	if(bridgeWin_d(b,code)) {
		return true;
	}
	if(forkWin_d(b,code)) {
		return true;
...
__device__ idx init_toplay_d(idx* toplay, const small_board * b) {
	idx lastIndex = 0;
	short i;
	for(i = 0; i < ELEMENT_SIZE; i++) {
		if(b->cell[i] != BLACK && b->cell[i] != WHITE) {
			toplay[lastIndex] = i;
			lastIndex++;
		}
	}
...
__device__ bool bridgeWin_d(small_board * b, const code_board * code) {
	...
	byte color = next_d(b);
	if(checkForBridge_d(b,code,color)) {
		setWinner_d(b, color);
		setReason_d(b, BRIDGE);
		return true;
	}
	return false;
}
...
__device__ bool checkForBridge_d(small_board * b, const code_board * code, byte colorOnMove) {
	if(b->cell[code->corners[0]] == colorOnMove) {
		idx root1 = findRoot_d(b, code->corners[0]);
		idx root2;
		if(b->cell[code->corners[1]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[1]);
			if(root1 == root2) {
				return true;
			}
		}
		if(b->cell[code->corners[2]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[2]);
			if(root1 == root2) {
				return true;
			}
		}
		if(b->cell[code->corners[3]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[3]);
			if(root1 == root2) {
				return true;
			}
		}
		if(b->cell[code->corners[4]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[4]);
			if(root1 == root2) {
				return true;
			}
		}
		if(b->cell[code->corners[5]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[5]);
			if(root1 == root2) {
				return true;
			}
		}
	...
	if(b->cell[code->corners[1]] == colorOnMove) {
		idx root1 = findRoot_d(b, code->corners[1]);
		idx root2;
		if(b->cell[code->corners[2]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[2]);
			if(root1 == root2) {
	...
	if(b->cell[code->corners[2]] == colorOnMove) {
		idx root1 = findRoot_d(b, code->corners[2]);
		idx root2;
		if(b->cell[code->corners[3]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[3]);
			if(root1 == root2) {
	...
	if(b->cell[code->corners[3]] == colorOnMove) {
		idx root1 = findRoot_d(b, code->corners[3]);
		idx root2;
		if(b->cell[code->corners[4]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[4]);
			if(root1 == root2) {
	...
	if(b->cell[code->corners[4]] == colorOnMove) {
		idx root1 = findRoot_d(b, code->corners[4]);
		idx root2;
		if(b->cell[code->corners[5]] == colorOnMove) {
			root2 = findRoot_d(b, code->corners[5]);
			if(root1 == root2) {
...
__device__ bool forkWin_d(small_board * b, const code_board * code) {
	// move is done, so check for next = prev player
	byte color = next_d(b);
	if(checkForFork_d(b,code,color)) {
		setWinner_d(b, color);
		setReason_d(b, FORK);
		return true;
	}
...
__device__ bool isConnectedToThreeEdges_d(idx index, short orginEdge, small_board * b, byte colorOnMove,const code_board * code) {
	...
	for(i=0; i < 6; i++) {
		if(i != orginEdge) {
			if(isConnectedToEdge_d(index,code->edges[i],b,colorOnMove)) {
				count++;
			}
		}
	}
...
__device__ bool isConnectedToEdge_d(idx index, edge edge, small_board * b, byte colorOnMove) {
	idx root1 = findRoot_d(b, index);
	int k;
	for(k = 0; k < size_d(b)-2; k++) {
		if(b->cell[edge.edge[k]] == colorOnMove) {
			idx root2 = findRoot_d(b, edge.edge[k]);
			if(root1 == root2) {
				return true;
			}
		}
...
__device__ bool ringWin_d(small_board * b, idx mv, const code_board * code) {
	byte color = onMove_d(b);
	if(checkForRing_d(b,mv,code,color)) {
		setWinner_d(b, color);
		setReason_d(b, RING);
		// for result only
		perf_move_d(b,mv,code);
		return true;
	}
	return false;
}
...
__device__ bool checkForRing_d(small_board * b, idx move, const code_board * code, byte color) {
	...
	for(i = 0; i < 6; i++) {
		...
		if(new_neighbour != NO_NEIGHBOUR && b->cell[new_neighbour] == color) {
			...
			if(checkForNearRing_d(b,code,new_neighbour,color)) {
				return true;
			}
		} else {
			n.neighbour[i] = NO_NEIGHBOUR;
	...
	if(n.neighbour[0] != NO_NEIGHBOUR) {
		idx root1 = findRoot_d(b, n.neighbour[0]);
		idx root2;
		if(n.neighbour[1] != NO_NEIGHBOUR) {
			if((n.neighbour[4] == NO_NEIGHBOUR || n.neighbour[5] == NO_NEIGHBOUR) && (n.neighbour[2] == NO_NEIGHBOUR || n.neighbour[3] == NO_NEIGHBOUR)) {
				root2 = findRoot_d(b, n.neighbour[1]);
				if(root1 == root2) {
					return true;
				}				
			}
		...
		if(n.neighbour[3] != NO_NEIGHBOUR) {
			if(n.neighbour[2] == NO_NEIGHBOUR && (n.neighbour[4] == NO_NEIGHBOUR||n.neighbour[5] == NO_NEIGHBOUR ||n.neighbour[1] == NO_NEIGHBOUR)) {
				root2 = findRoot_d(b,n.neighbour[3]);
				if(root1 == root2) {
					return true;
				}	
			}
		...
		if(n.neighbour[5] != NO_NEIGHBOUR) {
			if(n.neighbour[4] == NO_NEIGHBOUR&& (n.neighbour[3] == NO_NEIGHBOUR||n.neighbour[2] == NO_NEIGHBOUR ||n.neighbour[1] == NO_NEIGHBOUR)) {
				root2 = findRoot_d(b,n.neighbour[5]);
				if(root1 == root2) {
					return true;
				}
			}
	...
	if(n.neighbour[1] != NO_NEIGHBOUR) {
		idx root1 = findRoot_d(b, n.neighbour[1]);
		idx root2;
		if(n.neighbour[2] != NO_NEIGHBOUR) {
			if(n.neighbour[3] == NO_NEIGHBOUR&& (n.neighbour[4] == NO_NEIGHBOUR||n.neighbour[5] == NO_NEIGHBOUR ||n.neighbour[0] == NO_NEIGHBOUR)) {
				root2 = findRoot_d(b, n.neighbour[2]);
				if(root1 == root2) {
					return true;
				}
			}
		...
		if(n.neighbour[4] != NO_NEIGHBOUR) {
			if(n.neighbour[5] == NO_NEIGHBOUR&& (n.neighbour[0] == NO_NEIGHBOUR||n.neighbour[2] == NO_NEIGHBOUR ||n.neighbour[3] == NO_NEIGHBOUR)) {
				root2 = findRoot_d(b, n.neighbour[4]);
				if(root1 == root2) {
					return true;
				}
			}
	...
	if(n.neighbour[2] != NO_NEIGHBOUR) {
		idx root1 = findRoot_d(b, n.neighbour[2]);
		idx root2;
		if(n.neighbour[4] != NO_NEIGHBOUR) {
	...
	if(n.neighbour[3] != NO_NEIGHBOUR) {
		idx root1 = findRoot_d(b, n.neighbour[3]);
		idx root2;
		if(n.neighbour[4] != NO_NEIGHBOUR) {
...
__device__ bool checkForNearRing_d(small_board * b,const code_board * code,idx nb, byte color) {
	__shared__ neighbours nn;
	getNeighbours_d(code, nb, &nn);
	short j;
	short nc = 0;
	for(j = 0; j < 6; j++) {
